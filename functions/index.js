...
exports.sendBidNotificationEmail = functions.firestore
    ...
    .onWrite(async (change, context) => {
      ...
      const appId = context.params.appId; // Get appId from the path

      const oldBid = change.before.exists ? change.before.data() : null;
      const newBid = change.after.exists ? change.after.data() : null;
      ...
      const contactEmail = functions.config().contact.email; // FromFnctconfig
      const senderEmail = functions.config().resend.sender_email; // verfResend

      // Determine the action (create, update, delete) and gather data
      if (!change.after.exists) { // Delete operation
      ...
      `;
      } else if (!change.before.exists) { // Create operation
      ...
      <ul>
        ...
        <li><b>Action:</b> ${!change.after.exists ?
          "Withdrawal" :
          (!change.before.exists ? "New Bid" : "Update Bid")}</li>
      </ul>
      ${emailContent}
    `;
      await sendEmailResend(
